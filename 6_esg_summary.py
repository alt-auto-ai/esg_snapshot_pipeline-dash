...
parser = argparse.ArgumentParser(
    description="For rows with ESG_or_not='Yes', generate an ESG draft and write adjacent column 'ESG Summary'."
)
...
# 🔁 Read from 5_story_jurisdiction.csv
INPUT_CSV = os.getenv(
    "INPUT_CSV",
    r"5_story_jurisdiction.csv"
)
# 🆕 Write to 6_ESG_Summary.csv
...
def load_profile_config(profile: str):
    ...
    cfg = {
        ...
        # Default prompt file name now targets ESG drafting
        "PROMPT_FILE": get_env(f"{px}_PROMPT_FILE", "6_esg_summary.yaml"),
    }
...
# ----------------- Prompt loading -----------------
# 🔁 Point to 6_ESG_Summary.yaml
PROMPT_FILE_PATH = r"6_esg_summary.yaml"
...
# Render prompt with the markdown file contents
...
async def process_file_async(session, md_file, sem, idx, total):
    async with sem:  # local concurrency limiter
        ...
        if not os.path.exists(md_path):
            ...
            return {"md_file": md_file, "ESG_Summary": ""}
        ...
        try:
            ...
            return {"md_file": md_file, "ESG Summary": ""}

        if not raw.strip():
            print(f"   ⚠️ [{md_file}] Empty response.")
            return {"md_file": md_file, "ESG Summary": ""}

        draft = strip_code_fences(raw).strip()
        print(f"   📝 [{md_file}] Draft length={len(draft)} chars.")
        return {"md_file": md_file, "ESG Summary": draft}
...
async def main_async():
    ...
    # Read input CSV: Date, Title, URL, md_file, ESG_or_not, Jurisdiction (may include others)
    ...
    normalized_fields = {fn.strip().lower(): fn for fn in fieldnames}
    required_cols = {"date", "title", "url", "md_file"}
    if not input_rows or not required_cols.issubset(set(normalized_fields.keys())):
        raise SystemExit("[INPUT ERROR] CSV must have columns at least: Date, Title, URL, md_file (case-insensitive).")

    esg_field = normalized_fields.get("esg_or_not")
    if not esg_field:
        raise SystemExit("[INPUT ERROR] Could not locate 'ESG_or_not' column (case-insensitive search).")
    ...
    candidates = [
        ...
        if (r.get(esg_field) or "").strip().lower() == "yes"
    ]
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if tasks:
            ...
            for r in results:
                draft_map[r["md_file"]] = r.get("ESG Summary", "")
        else:
            print("[INFO] No ESG='Yes' rows to draft.")

    # Merge back: write ALL original columns + ESG Summary (appended at end)
    print("\n📁 Writing results to CSV...")
    fieldnames_out = list(fieldnames)
    if "ESG Summary" not in fieldnames_out:
        fieldnames_out.append("ESG Summary")
    ...
    with open(OUTPUT_CSV, "w", encoding="utf-8", newline="") as f:
        ...
        for row in input_rows:
            ...
            esg = (row.get(esg_field) or "").strip()
            draft = draft_map.get(md_file, "") if esg.lower() == "yes" else ""
            out_row = {k: (row.get(k) or "").strip() for k in fieldnames}
            out_row["ESG Summary"] = draft
