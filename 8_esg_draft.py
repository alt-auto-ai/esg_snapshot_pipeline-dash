...
# ----------------- Prompt loading -----------------
# 🔁 Use 8_esg_draft.yaml
PROMPT_FILE_PATH = r"8_esg_draft.yaml"
...
STRUCTURED_RESPONSE_FORMAT = {
    ...
    "json_schema": {
        ...
        "schema": {
            ...
            "required": ["headline", "points", "explainer"]
        }
    }
}
...
async def process_file_async(session, row, sem, idx, total):
    async with sem:
        ...
        if not os.path.exists(md_path):
            ...
            return {"md_file": md_file, "Headline": "", "Point_1": "", "Point_2": "", "Point_3": "", "Explainer": ""}
        ...
        headline = (obj.get("headline") or "").strip() if isinstance(obj, dict) else ""
        points = obj.get("points") if isinstance(obj, dict) else []
        if not isinstance(points, list): points = []
        p1 = (points[0] if len(points) > 0 else "").strip()
        p2 = (points[1] if len(points) > 1 else "").strip()
        p3 = (points[2] if len(points) > 2 else "").strip()
        explainer = (obj.get("explainer") or "").strip() if isinstance(obj, dict) else ""

        print(f"   📝 [{md_file}] Parsed: headline len={len(headline)}, p1/2/3 lens={[len(p1),len(p2),len(p3)]}, explainer len={len(explainer)}")
        return {"md_file": md_file, "Headline": headline, "Point_1": p1, "Point_2": p2, "Point_3": p3, "Explainer": explainer}
...
async def main_async():
    ...
    required_cols = {"Date", "Title", "URL", "md_file", "ESG_Relevance"}
    if not input_rows or not required_cols.issubset(set(fieldnames)):
        raise SystemExit("[INPUT ERROR] CSV must include: Date, Title, URL, md_file, ESG_Relevance")
    ...
    # Candidates: ESG_Relevance == 'High' AND target columns empty (so we don't overwrite)
    target_cols = ["Headline", "Point_1", "Point_2", "Point_3", "Explainer"]
    candidates = []
    for r in input_rows:
        relevance = (r.get("ESG_Relevance") or "").strip().lower()
        ...
        if relevance == "high" and has_md and empties:
            candidates.append(r)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if tasks:
            ...
            print("[INFO] No rows to draft (ESG_Relevance != 'High' or outputs already filled).")

    # Merge back: write ALL original columns + structured columns (append if missing)
    print("\n📁 Writing results to CSV...")
    fieldnames_out = list(fieldnames)
    for col in ["Headline", "Point_1", "Point_2", "Point_3", "Explainer"]:
    ...
    with open(OUTPUT_CSV, "w", encoding="utf-8", newline="") as f:
        ...
        for row in input_rows:
            md_file = (row.get("md_file") or "").strip()
            relevance = (row.get("ESG_Relevance") or "").strip().lower()
            out_row = {k: (row.get(k) or "").strip() for k in fieldnames}

            if md_file in results_map and relevance == "high":
